...
// Props:
//   patientMeta      — current IPFS metadata object (already fetched by parent)
//   onUploadComplete — called after successful on-chain update so parent can refetch
// ─────────────────────────────────────────────────────────────────────────────
function PatientDocumentUpload({ patientMeta, onUploadComplete }) {
  const [open, setOpen] = useState(false);

  // ── File lists ───────────────────────────────────────────────────────────
  const [medFiles,  setMedFiles]  = useState([]); // { file, name }
  const [presFiles, setPresFiles] = useState([]); // { file, name }
  ...
  const handleUpload = async () => {
    ...
    try {
      // ── Upload each medical record file ──────────────────────────────────
      const uploadedMedRecords = [...(patientMeta?.medicalRecordFiles || [])];
      for (let i = 0; i < medFiles.length; i++) {
        ...
        uploadedMedRecords.push({
          cid,
          name: medFiles[i].name,
          uploadedAt: new Date().toISOString(),
        });
      }

      // ── Upload each prescription file ────────────────────────────────────
      const uploadedPrescriptions = [...(patientMeta?.prescriptionFiles || [])];
      ...
      // ── Update on-chain ───────────────────────────────────────────────────
      setUploadStatus('Confirm in MetaMask...');
      writeContract({
        address: CONTRACT_ADDRESS,
        abi: CONTRACT_ABI,
        functionName: 'updatePatientMetadata',
        args: [newMetadataHash],
        gas: 5000000n,
      });
  ...
  // Notify parent when tx confirmed
  if (isSuccess && onUploadComplete) {
    onUploadComplete();
  }

  // ── Existing uploaded files from IPFS ────────────────────────────────────
  const existingMedRecords    = patientMeta?.medicalRecordFiles  || [];
  const existingPrescriptions = patientMeta?.prescriptionFiles   || [];
        ...
        <div className="mt-4 bg-white rounded-2xl border-2 border-purple-100 shadow-xl overflow-hidden"
          ...
          <div className="bg-gradient-to-r from-purple-600 to-blue-600 px-6 py-4">
            ...
            <p className="text-purple-100 text-sm mt-0.5">
              Upload your previous medical records and prescriptions. These will be stored on IPFS
              and visible to doctors you grant access to.
            </p>
          </div>
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="pt-4 border-t border-gray-100">
              <div className="flex items-center justify-between mb-3">
                ...
                <input
                  ref={presRef} type="file" multiple
                  accept=".pdf,.jpg,.jpeg,.png,.doc,.docx"
            ...
            {/* ── Upload button ── */}
            {(medFiles.length > 0 || presFiles.length > 0) && !isSuccess && (
              <button
              ...
              >
                {uploading || isLoading ? (
                  ...
                  <>
                    <span>📤</span>
                    Upload {medFiles.length + presFiles.length} File{medFiles.length + presFiles.length > 1 ? 's' : ''} to IPFS
                  </>
                )}
              </button>
            )}
